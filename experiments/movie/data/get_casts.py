...
def worker(api_key, movies, wfile):
  language="en-US"
  casts = []
  cast_ids = set()
  for movie in movies:
  ...
  for cast_id in cast_ids:
    ...
    counter += 1
    ## Save casts every 100 for incrementality
    if counter % 100 == 0:
      with open(wfile, "w") as cast_file:
        json.dump(casts, cast_file, indent=2)
      print("Successfully saved first", counter)
...
def main():
  ...
  with open(options.rfile, "r") as movie_file:
    ...
    casts = worker(options.api_key, movies, options.wfile)
